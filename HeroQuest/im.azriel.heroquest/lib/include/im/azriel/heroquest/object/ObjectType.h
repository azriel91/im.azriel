...
private:
	 ...
	 */
	const config::Header* const header;
	/**
	 * The sequences defined for this widget.
	 */
	const vector<const config::Sequence*>* const sequences;
...
public:
	 ...
	 * @return the header
	 */
	const config::Header* getHeader() const;
	/**
	 ...
	 */
	const vector<const config::Sequence*>* getSequences() const;

private:
	ObjectType(const int id, const config::Header* const header,
	        const vector<const config::Sequence*>* const sequences);
...
private:
	static const config::Header* deserializeHeader(const xml_node node);
	static const config::ImageMetadata* deserializeImageMetadata(const xml_node node);
	static const config::Sequence* deserializeSequence(const xml_node node);
	static const config::Frame* deserializeFrame(const xml_node node);
	static const config::ImageReference* deserializeImageReference(const xml_node node);
};
